# CAMP mOTUs profiling


[![Documentation Status](https://img.shields.io/readthedocs/camp_motus-profiler)](https://camp-documentation.readthedocs.io/en/latest/motus-profiler.html)
...
2. Set up the conda environment (contains Snakemake, Click, and other essentials) using `configs/conda/motus-profiler.yaml`. 
```Bash
...
python /path/to/camp_motus-profiler/workflow/motus-profiler.py test
```
...
- `/path/to/work/dir/motus-profiler/final_reports/samples.csv` for ingestion by the next module (ex. quality-checking)
<!--- 
...
└── workflow
    ...
    └── ext/
        └── scripts/
```
- `workflow/motus-profiler.py`: Click-based CLI that wraps the `snakemake` and other commands for clean management of parameters, resources, and environment variables.
...
```Bash
python /path/to/camp_motus-profiler/workflow/motus-profiler.py \
...
#!/bin/bash
python /path/to/camp_motus-profiler/workflow/motus-profiler.py --slurm \
...
```Bash
python3 /path/to/camp_motus-profiler/workflow/motus-profiler.py cleanup \
    -d /path/to/work/dir \
    -s /path/to/samples.csv
```
...
```Bash
python3 /path/to/camp_motus-profiler/workflow/motus-profiler.py --dry_run \
    -d /path/to/work/dir \
    -s /path/to/samples.csv
```
